...
export default class LoginFormUiHandler extends FormModalUiHandler {
  ...
  getFields(config?: ModalConfig): string[] {
    return [ i18next.t("menu:username"), i18next.t("menu:password"), i18next.t("menu:invitation") ];
  }
  ...
  show(args: any[]): boolean {
    if (super.show(args)) {
      ...
      this.submitAction = (_) => {
        ...
        Utils.apiPost("account/login", `username=${encodeURIComponent(this.inputs[0].text)}&password=${encodeURIComponent(this.inputs[1].text)}&invitation=${encodeURIComponent(this.inputs[2].text)}`, "application/x-www-form-urlencoded")
          .then(response => {
            if (!response.ok) {
